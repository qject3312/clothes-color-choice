...
from backend.schemas import ClothingCreate, ClothingUpdate, OutfitEvaluateRequest, SavedOutfitCreate, UserCreate, UserLogin
...
def update_clothing(clothing_id: int, clothing: ClothingUpdate):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        """
        UPDATE clothes
        SET category = ?, detail = ?, feature = ?, color_name = ?, color_hex = ?
    ...
    if updated == 0:
        raise HTTPException(status_code=404, detail="수정할 옷을 찾을 수 없습니다.")
...
def add_saved_outfit(data: SavedOutfitCreate):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO saved_outfits (user_id, outfit_data) VALUES (?, ?)",
        (data.user_id or "guest", json.dumps(data.outfit_data, ensure_ascii=False)),
    )
    new_id = cur.lastrowid
    conn.commit()
    conn.close()
    return {"message": "코디가 저장되었습니다.", "id": new_id}


@app.get("/saved-outfits")
def get_saved_outfits(user_id: str = Query("guest")):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        "SELECT id, outfit_data, created_at FROM saved_outfits WHERE user_id = ? ORDER BY id DESC",
        (user_id,),
    )
    rows = cur.fetchall()
    conn.close()
    results = []
    for row in rows:
        outfit = json.loads(row["outfit_data"])
        outfit["id"] = row["id"]
        outfit["created_at"] = row["created_at"]
        results.append(outfit)
    return results


@app.delete("/saved-outfits/{outfit_id}")
def delete_saved_outfit(outfit_id: int, user_id: str = Query("guest")):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("DELETE FROM saved_outfits WHERE id = ? AND user_id = ?", (outfit_id, user_id))
    deleted = cur.rowcount
    conn.commit()
    conn.close()
    if deleted == 0:
        raise HTTPException(status_code=404, detail="삭제할 코디를 찾을 수 없습니다.")
    return {"message": "삭제되었습니다."}
