...
def score_temperature(outfit, temp):
    ...
    for part, item in outfit.items():
        ...
        if temp >= 28 and any(k in text for k in ["패딩", "코트", "니트", "기모", "후리스"]):
            score -= 8
            reasons.append("더운 날씨에는 두꺼운 소재가 부담스러울 수 있습니다.")
        if temp <= 10 and any(k in text for k in ["반팔", "민소매", "반바지", "얇은"]):
            score -= 8
            reasons.append("추운 날씨에는 얇은 옷 단독 착용이 부족할 수 있습니다.")
    ...
    if temp <= 16 and has_outer:
        ...
        score -= 3
        reasons.append("더운 날씨라 아우터는 다소 답답할 수 있습니다.")
...
def calculate_outfit_score(user, outfit, temp=22):
    ...
    total = color + weather + body + personal + style + complete
...
def recommend_outfits(user, clothes, temp=22, top_n=3):
    """모든 조합을 점수화하여 TOP N 추천 결과 반환"""
    candidates = generate_outfit_candidates(clothes)
    results = []

    for outfit in candidates:
        score = calculate_outfit_score(user, outfit, temp=temp)
        results.append({
            "outfit": outfit,
            "score": score["total"],
    ...
    # 같은 상의/하의 조합이 반복되는 것 방지
    filtered = []
    used_pairs = set()
    for result in results:
        top_id = get_value(result["outfit"].get("top"), "id", default=id(result["outfit"].get("top")))
        bottom_id = get_value(result["outfit"].get("bottom"), "id", default=id(result["outfit"].get("bottom")))
...
def build_recommendation_summary(outfit, score_info):
    reasons = []
    for _, data in score_info["details"].items():
        if data["reasons"]:
            reasons.append(data["reasons"][0])
    return reasons[:5]
...
def extract_dominant_colors(image_path, color_count=3):
    ...
    # 옷 사진은 보통 중앙에 있으므로 중앙 70%만 우선 분석
    crop_ratio = 0.70
    ...
    all_pixels = list(center_img.getdata())
    colorful_pixels = []
    neutral_pixels = []
