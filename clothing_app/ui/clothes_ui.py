...
class ClothesUI:
    ...
    def edit_cloth(self, cloth, refresh_callback):
        ...
        body = tk.Frame(edit_win, bg=style.CARD)
        body.pack(fill="both", expand=True, padx=22)

        def label(text):
            tk.Label(body, text=text, font=(style.FONT, 10, "bold"), bg=style.CARD, fg=style.TEXT).pack(anchor="w", pady=(10, 4))
        ...
        detail_box.pack(fill="x", ipady=4)
        ...
        color_entry.pack(fill="x", ipady=6)
        ...
        feature_entry.pack(fill="x", ipady=6)
        ...
        style.button(body, "수정 저장", save_edit, bg=style.PRIMARY, pady=(18, 5))
        style.ghost_button(body, "취소", edit_win.destroy, pady=5)
