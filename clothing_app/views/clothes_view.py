...
from views.components import (card, top_bar, btn_primary, btn_secondary, section,
...
def _get_uid(app_state):
    user = app_state.get("user") or app_state.get("current_user")
    return user.get("user_id", "guest") if isinstance(user, dict) else "guest"
...
def build_clothes(page, go, app_state):
    ...
    def make_cloth_card(cloth):
        ...
        def on_delete(_):
            def confirm(e):
                ...
                if cloth_id is not None:
                    delete_clothing_backend(cloth_id, _get_uid(app_state))
        ...
        def on_edit(_):
            # Flet session storage serializes values and cannot store Clothing objects.
            app_state["edit_cloth"] = cloth
            go("/clothes_edit")
...
def build_clothes_edit(page, go, app_state):
    """옷 정보 수정 화면."""
    cloth = app_state.get("edit_cloth")
    if not cloth:
    ...
    def on_save(_):
        ...
        if cid is not None:
            result = update_clothing_backend(cid, cloth, _get_uid(app_state))
