...
def button_with_text(root, text):
    for control in walk_controls(root):
        ...
        values.append(getattr(control, "text", "") or "")
...
class FeatureFixTests(unittest.TestCase):
    ...
    def test_custom_recommend_save_uses_flat_serializable_outfit(self):
        page = FakePage()
        clothes = [
            Clothing("상의", "반팔", "캐주얼", (255, 0, 0), "#ff0000", "빨강", clothing_id=1),
            Clothing("하의", "청바지", "캐주얼", (0, 0, 255), "#0000ff", "파랑", clothing_id=2),
        ]
        state = {"user": {"user_id": "tester"}, "clothes": clothes}
        captured = {}
        original = recommend_view.save_outfit_backend
        recommend_view.save_outfit_backend = (
            lambda outfit, uid: captured.update(outfit=outfit, uid=uid) or {"id": 11}
        )
        try:
            screen = build_recommend(page, lambda _route: None, state)
            button_with_text(screen, "코디 저장").on_click(None)
        finally:
            recommend_view.save_outfit_backend = original

        saved = captured["outfit"]
        self.assertEqual(captured["uid"], "tester")
        self.assertNotIn("outfit", saved)
        self.assertEqual(saved["top"]["detail"], "반팔")
        self.assertEqual(saved["bottom"]["detail"], "청바지")
        self.assertEqual(saved["id"], 11)
        self.assertIs(state["saved_outfits"][0], saved)
