...
def _serialize_outfit(outfit):
    ...
    for k, v in outfit.items():
        if k in cloth_keys:
            ...
            result[k] = [_serialize_cloth(item) for item in (v or [])]
        else:
            result[k] = v
    return result
...
def item_row(label, cloth):
    if isinstance(cloth, (list, tuple)):
        items = [item for item in cloth if item is not None]
        if not items:
            return item_row(label, None)
        return ft.Column(
            [item_row(label if index == 0 else "", item)
    ...
    if cloth is None:
        return ft.Row([
            ft.Container(ft.Text(label_text, size=F["sm"], weight=ft.FontWeight.BOLD,
...
def build_recommend(page, go, app_state):
    ...
    recommend_round = int(app_state.get("_recommend_round", 0))
    ...
    def do_recommend(advance=False):
        nonlocal recommend_round
        if advance:
            recommend_round += 1
            app_state["_recommend_round"] = recommend_round
    ...
    def make_result_card(rank, result):
        ...
        def do_save(_):
            saved = _serialize_outfit(outfit)
            saved["score"] = score
            saved["reason"] = " ".join(reasons) or "맞춤 코디 추천"
                ...
                card(ft.Column([
                    ...
                    ft.Row([
                        ft.Text("현재 기온(℃)", size=F["sm"],
                                weight=ft.FontWeight.BOLD, color=C["text"]),
                        temp_tf,
                        btn_primary("다시 추천", lambda e: do_recommend(advance=True),
...
def build_personal_recommend(page, go, app_state):
    ...
    round_index = int(app_state.get("_personal_recommend_round", 0))
    ...
    def pick_best(category):
        ...
        ranked = sorted(cats, key=lambda c: calc_score(c)[0], reverse=True)
        best = ranked[round_index % len(ranked)]
    ...
    def recommend_again(_):
        app_state["_personal_recommend_round"] = round_index + 1
        go("/personal")
    ...
    return ft.Column([
        ...
        ft.Container(
            content=ft.Column([
                ...
                btn_primary("다른 옷으로 다시 추천", recommend_again,
...
def build_coordination(page, go, app_state):
    selected = {"top":None,"bottom":None,"outer":None,"shoes":None,"accessory":[]}
    ...
    def refresh():
        ...
        for key, title, cat in cat_map:
            ...
            for cloth in cl:
                is_sel = (
                    any(item is cloth for item in selected[key])
                    if key == "accessory"
                    else selected[key] is cloth
                )
                def make_choose(k, c):
                    def choose(_):
                        if k == "accessory":
                            if any(item is c for item in selected[k]):
                                selected[k] = [item for item in selected[k] if item is not c]
                            else:
                                selected[k].append(c)
                        else:
    ...
    def do_save(_):
        ...
        if selected["accessory"]:
            accessory_names = ", ".join(_get(item, "detail") for item in selected["accessory"])
            parts.append(f"악세서리로 {accessory_names}를 더했습니다.")
        outfit_raw = {
            "top": selected["top"],
            "bottom": selected["bottom"],
            "outer": selected["outer"],
            "shoes": selected["shoes"],
            "accessory": selected["accessory"][0] if selected["accessory"] else None,
            "accessories": selected["accessory"],
        }
        outfit = _serialize_outfit(outfit_raw)
    ...
    return ft.Column([
        ...
        ft.Container(
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([
                        btn_primary("선택 완료 및 저장", do_save,
                                    ft.Icons.CHECK_CIRCLE_ROUNDED, C["success"]),
                        btn_primary("나의 코디 확인", lambda e: go("/outfit"),
...
def build_today(page, go, app_state):
    ...
    round_index = int(app_state.get("_today_recommend_round", 0))

    def pick_best(cat):
        cats = [c for c in clothes if _get(c,"category") == cat]
        if not cats: return None, 0, []
        ranked = sorted(cats, key=lambda c: score_cloth(c)[0], reverse=True)
        best = ranked[round_index % len(ranked)]
    ...
    def recommend_again(_):
        app_state["_today_recommend_round"] = round_index + 1
        go("/today")
