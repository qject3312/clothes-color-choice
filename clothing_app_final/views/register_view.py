...
try:
    ...
    def make_relative_to_app(p): return str(p)
...
def build_register_photo(page, go, app_state):
    ...
    eyedropper_text = ft.Text(
        "스포이드를 켠 뒤 사진에서 원하는 색을 누르면 직접 추가됩니다.",
        size=F["xs"], color=C["subtext"],
    )
    ...
    def add_manual_color(rgb):
        hex_code = rgb_to_hex(*rgb).upper()
        color = {"rgb": rgb, "hex": hex_code, "name": rgb_to_name(*rgb)}
        photo_state["colors"] = [c for c in photo_state["colors"] if c.get("hex") != hex_code]
        if len(photo_state["colors"]) >= photo_state["max_colors"]:
            photo_state["colors"].pop()
        photo_state["colors"].append(color)
        refresh_colors()

    def sample_photo_color(e):
        if not photo_state["eyedropper"] or not photo_state["file_path"]:
            return
        try:
            from PIL import Image

            with Image.open(photo_state["file_path"]).convert("RGB") as img:
                iw, ih = img.size
                scale = min(260 / iw, 260 / ih)
                shown_w, shown_h = iw * scale, ih * scale
                offset_x, offset_y = (260 - shown_w) / 2, (260 - shown_h) / 2
                local_x = float(getattr(e, "local_x", -1))
                local_y = float(getattr(e, "local_y", -1))
                if not (offset_x <= local_x <= offset_x + shown_w and
                        offset_y <= local_y <= offset_y + shown_h):
                    return
                x = min(iw - 1, max(0, int((local_x - offset_x) / scale)))
                y = min(ih - 1, max(0, int((local_y - offset_y) / scale)))
                add_manual_color(tuple(img.getpixel((x, y))))
                eyedropper_text.value = f"선택 완료: {photo_state['colors'][-1]['name']} {photo_state['colors'][-1]['hex']}"
        except Exception as ex:
            eyedropper_text.value = f"색상 선택 실패: {ex}"
        page.update()

    def toggle_eyedropper(_):
        if not photo_state["file_path"]:
            page.open(ft.SnackBar(ft.Text("먼저 사진을 선택해 주세요."), bgcolor=C["danger"]))
            return
        photo_state["eyedropper"] = not photo_state["eyedropper"]
        eyedropper_text.value = (
            "스포이드가 켜졌습니다. 사진에서 원하는 색을 눌러 주세요."
            if photo_state["eyedropper"]
            else "스포이드가 꺼졌습니다."
        )
        eyedropper_text.color = C["primary"] if photo_state["eyedropper"] else C["subtext"]
        page.update()

    image_display.on_tap_down = sample_photo_color
    ...
    def on_file_pick(e):
        ...
        # 파일명(청바지, 반팔, 가방 등)을 우선하고 실루엣을 보조로 사용합니다.
        guess = infer_clothing_type(photo_state["file_path"])
        if guess and guess.get("category") in DETAIL_OPTIONS:
            category_dd.value = guess["category"]
            update_details()
            if guess.get("detail") in DETAIL_OPTIONS.get(category_dd.value, []):
                detail_dd.value = guess["detail"]
            auto_guess_text.value = (
                f"자동 추측: {guess['category']} / {guess['detail']} · {guess.get('reason', '')}"
            )
        else:
            auto_guess_text.value = "종류를 확실히 추측하지 못했습니다. 아래에서 직접 선택해 주세요."
        page.update()
    ...
    def on_save(_):
        ...
        try:
            ...
            copied_path = os.path.join(image_dir, filename)
            shutil.copy2(photo_state["file_path"], copied_path)
            saved_image_path = make_relative_to_app(copied_path)
        except Exception as ex:
            print("이미지 복사 실패:", ex)
